...
const getReportedPosts = () => {
    return reportedPost.filter((post) => reportedPostsId.includes(post.id));
};
...
const reportPost = (id) => {
    ...
    posts = posts.filter((post) => !reportedPostsId.includes(post.id));
    showPosts(posts);
};
...
const createPost = (post) => {
    ...
    const userText = post.comments[0]?.text;
